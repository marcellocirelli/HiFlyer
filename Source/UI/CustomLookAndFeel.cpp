                                          ...
                                          juce::Slider& slider)
{
    ...
    const auto& filmstrip = isNotched ? notchedKnobFilmstrip : knobFilmstrip;

    const int frameId = static_cast<int> (std::floor (sliderPos * (static_cast<double> (knobNumFrames) - 1.0)));
    const int imgWidth = filmstrip.getWidth();
    const int imgHeight = filmstrip.getHeight() / knobNumFrames;
                                          ...
                                          juce::Slider::SliderStyle, juce::Slider& slider)
{
    ...
    const int defaultThumbWidth  = isSwitch ? 20 : 30;
    const int defaultThumbHeight = isSwitch ? 20 : 30;
