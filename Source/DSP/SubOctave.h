...
public:
    ...
    // For Growl
    float getCarrier() const noexcept { return subCarrierOut; }
...
private:
    ...
    // Input conditioning pulled from hardware
    static constexpr float kInputHighpassMs = 360.0f;
    static constexpr float kTrackingLowpassMs = 0.42f;

    // Envelope follower for comparator threshold and output level
    ...
    // Hardware buzz switch as low-pass/tone shaping, pulled from schematic
